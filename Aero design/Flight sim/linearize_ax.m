...
function [f,dfdh,dfdvy,dfdvx,dfdth,dfdw,dfda] = linearize_ax(ac_struct,h,vy,vx,th,w,a,rho,elev)
  dfdh = 0;
  dfdw = 0;
  dfdvy = 0;
  ...
  dfdvx = -(Sa*rho*vx/m)*((dClda*a+Cl0)*sin(th-a)+(dCdda*a+Cd0)*cos(th-a));
  dfdth = (Sa*rho*vx*vx/2/m)*((dCdda*a+Cd0)*sin(th-a)+(-dClda*a-Cl0)*cos(th-a));
  dfda = (Sa*rho*vx*vx/2/m)*((dCdda*a+dClda+Cd0)*sin(a-th)+(dClda*a+Cl0-dCdda)*cos(a-th));
